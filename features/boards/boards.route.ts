...
import { boardGetRequestSchema, boardPatchRequestSchema, boardPostRequestSchema } from "./boards.schema";
import { createBoard, getUserBoards,updateBoard } from "./boards.service";
...
boardsRouter.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    const { body,params:{id} } = boardPatchRequestSchema.parse(req);
